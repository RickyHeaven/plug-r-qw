<template>
	<div class="examplePageL vh100">
		<showReadMe />
		<toHome />
		<tableTree ref="tableTree" :columns="columns" :data="data" :radio="true" @on-select="onSelect" />
		<form-modal
			ref="formModal"
			:title="title"
			:form-data="formData"
	...
	export default {
										...
										on: {
											click: () => {
												this.messageBox({
													...
													onOk: () => {
														this.$refs.tableTree
															.deleteLocalNode(params.row._index)
															.then(() => {
		...
		methods: {
			...
			onSubmit(data) {
				if (this.action === 'new') {
					this.$refs.tableTree
						.addLocalNode(this.editIndex, data)
						.then((r) => {
					...
					this.$refs.tableTree
						.editLocalNode(this.editIndex, data)
						.then((r) => {
							this.setTimeout(() => {
								//模拟请求，展示loading功能
								this.$refs.formModal.close()
