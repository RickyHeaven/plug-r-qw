      ...
      <div class="wellsL" v-for="n in 8" :key="'useE'+n">
        <wellCard :title="'多维'+exName[Math.ceil(n/2)-1]+(n%2===1?'直接':'按条件')+'查找，返回'+(switchV[n-1]?'路径':'元素')">
          ...
          <div class="wellInnerK">
            <span>用法： </span>
            <span v-if="n%2===1">this.findCollection(this.exData.<b>{{exKey[Math.ceil(n/2)-1]}}</b>, <mark>XXX</mark>{{switchV[n-1]?', true':''}})</span>
            <span v-if="n%2===0">this.findCollection(this.exData.<b>{{exKey[Math.ceil(n/2)-1]}}</b>, <mark>e=>e[YYY]===XXX</mark>{{switchV[n-1]?', true':''}})</span>
...
<script>
  export default {
    ...
    methods: {
      find(action) {
        if (action % 2 === 1) {
          this.rVB[action] = this.findCollection(this.exData[this.exKey[Math.ceil(action / 2) - 1]], this.valV[action - 1],
            this.switchV[action - 1])
        }
        else {
          this.rVB[action] = this.findCollection(this.exData[this.exKey[Math.ceil(action / 2) - 1]],
            e => e[this.valA[action]] === this.valV[action - 1], this.switchV[action - 1])
        }
      }
    }
  }
