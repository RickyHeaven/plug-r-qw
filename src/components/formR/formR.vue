	...
	>
		...
		<div class="inlineBlock">
			...
			<Button
				v-if="showInlineOkBt"
				type="primary"
				:class="{ inlineFormBtXN: inline, okBtnXN: true }"
				@click="submit"
				:loading="btnLoading && showLoading"
				:disabled="disabled"
			...
			>
			<!--取消按钮（清除）-->
			<Button v-if="showInlineClearBt" @click="resetForm" :class="{ inlineFormBtXN: inline }" type="dashed">{{
				inlineClearBtTxt || t('r.clear')
			}}</Button>
		</div>
	</Form>
</template>
